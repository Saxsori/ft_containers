...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   binary_search_tree_map_set.hpp                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aaljaber <aaljaber@student.42abudhabi.a    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/20 16:09:17 by aaljaber          #+#    #+#             */
/*   Updated: 2023/02/25 15:56:03 by aaljaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef BINARY_SEARCH_TREE_SET_HPP
# define BINARY_SEARCH_TREE_SET_HPP
	...
	class binary_search_tree_set
	{
		...
		private:
			...
			ft::node<data_type>			*_findGreatGrandFather(ft::node<data_type> *node) const
			{
				...
				while (true)
				{
					if (node->parent && _comp(node->data, node->parent->data))
						return (node->parent);
					else if (!node->parent)
					{
						if (_comp(node->data, firstNode->data))
							return _pastTheEnd;
						else
							return node;
					}
			...
			ft::node<data_type>			*_findYoungGrandFather(ft::node<data_type> *node) const
			{
				...
				while (true)
				{
					if (node->parent && _comp(node->parent->data, node->data))
						return (node->parent);
					else if (!node->parent)
					{
						if (_comp(firstNode->data, node->data))
							return NULL;
						else
							return node;
					}
		...
		public:
			mutable int counter;
			binary_search_tree_set(void):_comp()
			{
			...
			binary_search_tree_set(const binary_search_tree_set &x):_nodeSearched(NULL),_root(NULL),_comp(),_size(0),_pastTheEnd(NULL)
			{
				_size = x._size;
				_comp = x._comp;
				_root = x._root;
			...
			void						swap(binary_search_tree_set &x)
			{
			...
			ft::node<data_type>			*decrementNodeByOne(ft::node<data_type> *node) const
			{
				...
				if (!_isNode(node->left) && node->parent && _comp(node->parent->data, node->data))
					return (node->parent);
				else if (!_isNode(node->left) && node->parent && _comp(node->data, node->parent->data))
			...
			ft::node<data_type>			*incrementNodeByOne(ft::node<data_type> *node) const
			{
				if (!node || node == _pastTheEnd)
					...
					return (_pastTheEnd);
				if (!_isNode(node->right) && node->parent && _comp(node->data, node->parent->data))
					return (node->parent);
				else if (!_isNode(node->right) && node->parent && _comp(node->parent->data, node->data))
